...
export function SolarIqIntroSplit({
  observeTargetRef,
  children,
  hostClassName,
}: SolarIqIntroSplitProps) {
  const resolvedHost = hostClassName ?? (children ? defaultCustomHost : defaultIntroHost);

  return (
    <SolarIqLineSplitReveal observeTargetRef={observeTargetRef} hostClassName={resolvedHost}>
      {children ?? (
        <>
